...
public class AutoreController {
    ...
    @PostMapping("")
    public String saveBlog(@RequestBody @Validated List<Autore> autore, BindingResult bindingResult){
    ...
    @PutMapping("/{id}")
    public String updateAutore(@PathVariable int id, @RequestBody @Validated Autore autore,BindingResult bindingResult){
        if (bindingResult.hasErrors()){
            throw new BlogeAutoreEccezione("Richiesta non valida: "+bindingResult.getAllErrors().stream().map(e->e.getDefaultMessage()).reduce("",(s1,s2)->s1+"\n"+s2));
        }
